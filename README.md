...
| **1111316144** | **蔡政洋** | **`data_loader.py` & `cleaner.py` (資料流與清洗)**<br>負責實作原始 CSV 資料流載入，編寫自動辨識並切換 UTF-8 與 Big5 編碼的防崩潰機制；處理金額與日期的欄位型態轉換（`pd.to_numeric`, `pd.to_datetime`）並過濾髒數據。 | **34%** |
